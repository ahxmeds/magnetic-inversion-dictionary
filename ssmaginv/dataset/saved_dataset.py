...
from ssmaginv.config import TEST_DIR, TRAIN_DIR, VALID_DIR
...
def check_dataset_available(directory):
    ...
    if not os.path.exists(directory) or not any(
        f.endswith(".npz") for f in os.listdir(directory)
    ):
        print(f"[WARNING] Dataset directory '{directory}' is empty or missing.")
        print(
            "To generate the dataset npz files first run the script `dataset/generate_data.py`"
        )
        return False
    return True
